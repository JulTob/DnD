# 📝 Lines Annex

> 📚 **Record.** A report, an index or a working document, not a specification.

*Every drafted inspiration line (the sugar lines), gathered from the element
pages for wiring. Grouped by page and by the heading its table sits under.
Regenerate rather than edit.*
*627 lines.*


## Aasimar

**11. Lines**

- Darkvision: *Darkness cannot hide the truth from you.*
- Celestial Revelation, Talarian Wings: *You spread your wings to show your true self.*
- Monk's Focus, Aasimar only (proposal): *It comes down from the ring above your head and out through your hands. The body was always the instrument. You only had to consecrate it.*
- Celestial Patron, Aasimar only (proposal, for the ancestor synergy): *You know exactly who hired you. You have their wings.*

## Artificer

**Core lessons**

- Spellcasting (1): *The mystery has a workbench. You keep it tidy.*
- Tinker's Magic (1): *The smallest wonder goes into an object, not into your hand. Then it goes into a pocket, and it is somebody else's.*
- Replicate Magic Item (2): *A miracle that repeats is called a method. These are the ones you have written down.*
- Magic Item Tinker (6): *One more wonder at a time, including other people's.*
- Flash of Genius (7): *Your understanding, thirty feet away, on somebody else's roll.*
- Magic Item Adept (10): *Faster and cheaper is not a compromise. It is what a method is for.*
- Spell-Storing Item (11): *The spell in the tool, and the tool in a friend's hand, and you elsewhere. That is the whole point.*
- Magic Item Master (14): *Seven at once. The bench has become a person.*
- Advanced Artifice (18): *The tools are the magic now. There was never anything else on the bench.*
- Soul of Artifice (20): *The work always takes something back. You named the price in advance, and today it is paid.*
**Alchemist**

- Tools of the Trade (3): *Retorts, reagents, and the smell that never quite leaves your coat.*
- Alchemist Spells (3): *Cure and toxin on the same shelf. They always were.*
- Experimental Elixir (3): *Every batch comes out a little different. A result you expected teaches you nothing.*
- Alchemical Savant (5): *Your understanding goes into the flask with everything else.*
- Restorative Reagents (9): *Whoever drinks your morning's experiment walks away stronger.*
- Chemical Mastery (15): *You have tasted most of them yourself. Your body stopped complaining years ago.*
**Armorer**

- Tools of the Trade (3): *Heavy plate and the hammer that shapes it. You needed both.*
- Arcane Armor (3): *You made the knight you needed and stepped inside.*
- Armor Model (3): *Some days the work needs a wall. Some days a ghost. Same steel.*
- Armorer Spells (3): *What the suit learned to do while you were in it.*
- Extra Attack (5): *The suit answers twice.*
- Improved Armorer (9): *Rebuilt, again, by morning.*
- Perfected Armor (15): *Even the panic button shields somebody else. You designed it that way.*
**Artillerist**

- Tools of the Trade (3): *Wood, brass, and a chisel. Everything else is chemistry.*
- Artillerist Spells (3): *Fire, force, and a wall. The three things an engine is for.*
- Eldritch Cannon (3): *A squat little engine set down exactly where it matters. It barks, it hurls, it shelters.*
- Arcane Firearm (5): *Sigils in the stock. The barrel argues harder now.*
- Explosive Cannon (9): *It can also go off. You built that in, and you are a little proud of it.*
- Fortified Position (15): *Your friends stand behind what you made. Two of it, now.*
**Battle Smith**

- Tools of the Trade (3): *A sword, and the tools to keep a friend standing.*
- Battle Ready (3): *Your strikes land where the geometry says they must.*
- Battle Smith Spells (3): *Protection is a craft. These are its tools.*
- Steel Defender (3): *You built a friend. It has a gait you would know in the dark.*
- Extra Attack (5): *The two of you, twice.*
- Arcane Jolt (9): *The same current that sharpens the blade closes a wound.*
- Improved Defender (15): *It holds the line your mind measured, and now it answers the blow.*

## Barbarian

**Core lessons**

- Rage (1): *It was there before you had a word for it. Now you have one, and it comes when you call.*
- Weapon Mastery (1): *The weapon does not matter. The hand does. But you did learn what a weapon wants to do.*
- Danger Sense (2): *The hair on your arms knows before you do. You stopped arguing with it.*
- Reckless Attack (2): *Guarding is hesitation with a shield. Drop it, and see how fast the fight ends.*
- Primal Knowledge (3): *Rage is a sense. It hears the twig, smells the fear, and walks in without asking.*
- Extra Attack (5): *One blow was never going to be enough. It never is.*
- Fast Movement (5): *Everything between you and the fight is in the way.*
- Feral Instinct (7): *You were already moving when the others were still deciding.*
- Instinctive Pounce (7): *The Rage does not arrive. It lands.*
- Brutal Strike (9): *You can afford to be precise now, because you are not afraid of missing.*
- Relentless Rage (11): *Death has a door. You have never once used a door.*
- Persistent Rage (15): *It stopped needing an occasion. You are simply like this now, and it costs you nothing.*
- Primal Champion (20): *You are more than your people made. Whatever you were carrying, this is what it was for.*
**Berserker**

- Frenzy (3): *You gave up guarding. What you get back is everything else.*
- Intimidating Presence (14): *They see the whole of you at once, and most people have never seen the whole of anyone.*
**Wild Heart**

- Animal Speaker (3): *The wild does not speak. It answers. You learned to ask in the calm.*
- Rage of the Wilds (3): *Bear endures. Eagle goes. Wolf hunts for the pack. You knew which before you chose.*
- Aspect of the Wilds (6): *You borrowed a way of moving and forgot to give it back.*
- Nature Speaker (10): *Sit still long enough and the land will tell you what it is afraid of.*
**World Tree**

- Vitality of the Tree (3): *What flows through the tree flows through you, and you were never a bottle. You are a branch.*
- Branches of the Tree (6): *Distance is a story the world tells. You stopped listening.*
- Travel along the Tree (14): *You are not alone. So you do not cross alone.*
**Zealot**

- Divine Fury (3): *Whatever rides you brought its own fire. You only supply the hands.*
- Warrior of the Gods (3): *An instrument is kept in repair. That is not love. It is use, and you are grateful anyway.*
- Fanatical Focus (6): *The commandment does not care what you saw. Neither, for one breath, do you.*
- Zealous Presence (10): *When you shout, it is not your voice they hear. They stand up anyway.*

## Bard-and-Sorcerer-Lines

**The Bard: the told tale**

- Bardic Inspiration (1): *You say one thing at the right time, and somebody else is better for a moment. That is the whole art. The rest is practice.*
- Spellcasting (1): *A song is a spell that admits what it is. These are the ones that do not.*
- Expertise (2): *Two things you do the way other people breathe. Nobody applauds breathing.*
- Jack of All Trades (2): *You have done a little of everything, badly, in front of people. It adds up.*
- Font of Inspiration (5): *The well refills faster than it should. You have stopped asking whose well it is.*
- Countercharm (7): *Somebody else is singing in this room. You sing louder, and truer, and theirs breaks first.*
- Expertise (II) (9): *Two more. The list of things you are only good at keeps getting shorter.*
- Magical Secrets (10): *You heard it once, somewhere it was not meant for you, and you kept it. Collectors do.*
- Superior Inspiration (18): *The room turns and you already have the word ready. You always do now.*
- Words of Creation (20): *Two words, older than any song. You know them. You say them very seldom.*
- Dazzling Footwork (3): *No words. The room watches your feet instead and forgets to be hostile.*
- Inspiring Movement (6): *You move, and somebody who was stuck is not. It looks like nothing. It is the hardest thing you do.*
- Tandem Footwork (14): *Everyone finds their place a beat early, because you found yours.*
- Leading Evasion (14): *You step aside and the ones near you find they have too.*
- Beguiling Magic (3): *Beauty is a command. You have learned to give it.*
- Mantle of Inspiration (3): *For a moment they look the way you see them, and move like it.*
- Mantle of Majesty (6): *The room does what you say for a minute. It is not a spell. It is a spell.*
- Unbreakable Majesty (14): *Whoever raises a hand against you finds the hand has an opinion about it.*
- Bonus Proficiencies (3): *Three more things you know how to do because somebody once told you the story of doing them.*
- Cutting Words (3): *One word, placed exactly. It costs them the swing. It costs you nothing you will miss.*
- Magical Discoveries (6): *You found it in a footnote and it turned out to be real.*
- Peerless Skill (14): *When it matters, you know more than the roll does, and you add it.*
- Combat Inspiration (3): *The word you give them lands as a parry, or as the extra weight behind the blow.*
- Martial Training (3): *The deed and the telling are one craft. You learned the deed second.*
- Extra Attack (6): *Twice, because the story needs a second verse.*
- Battle Magic (14): *You cast, and then you hit, and both are the same sentence.*
**The Sorcerer: the magic felt before it is understood**

- Spellcasting (1): *It was in you before you had a word for it. The words came later, and they are still catching up.*
- Innate Sorcery (1): *You stop holding it in. For a minute the world notices.*
- Font of Magic (2): *There is more of it than the slots can hold. You have learned to pour between vessels.*
- Metamagic (2): *You do not cast the spell the way it is written. You cast it the way it feels.*
- Sorcerous Restoration (5): *A little comes back after a short rest. It never left; it was catching its breath.*
- Sorcery Incarnate (7): *Two ways at once to bend one spell. The body has stopped asking permission.*
- Arcane Apotheosis (20): *Whatever you were becoming, you have arrived. You still do not have the word.*
- Psionic Spells (3): *Some spells you do not say. They are simply true in the room.*
- Telepathic Speech (3): *You think at someone and they hear it. Neither of you likes that you can.*
- Psionic Sorcery (6): *No gesture, no word, no sign. Just the result, and the headache.*
- Psychic Defenses (6): *Whatever reached into you first left the door harder to open behind it.*
- Revelation in Flesh (14): *For a minute you show what you are. It has wings, or eyes, or no skin. Nobody asks twice.*
- Warping Implosion (18): *You step through somewhere that is not a place, and the place you left collapses to fill the hole.*
- Clockwork Spells (3): *Order came to you as a sound: a tick, under everything, that you can now make louder.*
- Restore Balance (3): *Someone's luck is running hot or cold. You set it back to even. It is almost a kindness.*
- Bastion of Law (6): *You build a wall out of the rule that things stay where they are put.*
- Trance of Order (14): *The tick becomes the only sound. Nothing surprises you in it.*
- Clockwork Cavalcade (18): *Everything in reach is set right at once, as if it had never been otherwise. The tick stops. Then starts.*
- Draconic Resilience (3): *Something under your skin has started to overlap.*
- Draconic Spells (3): *The spells that came first were the ones that burn, or freeze, or bite. You did not choose the order.*
- Elemental Affinity (6): *One element answers you the way a dog answers its name.*
- Dragon Wings (14): *They were always going to come out. You have stopped pretending you were surprised.*
- Dragon Companion (18): *Something with your element in its throat now follows you, and looks at you as if it knows.*
- Wild Magic Surge (3): *Sometimes, when you reach for it, it reaches back.*
- Tides of Chaos (3): *You lean on the chaos and it holds, once. Then it wants something.*
- Bend Luck (6): *You put a thumb on someone's scale. The scale notices.*
- Controlled Chaos (14): *Two doors open when you knock. You get to pick.*
- Tamed Surge (18): *It still reaches back. Now you choose which hand.*

## Bard

**3. The Muses: two open slots**

- Grace: *It steadies when you are forgiven, and it does not care whether you deserved it.*
**Core lessons**

- Bardic Inspiration (1): *You have seen what a song does to a room. Now you can aim it.*
- Spellcasting (1): *The act is the spell. There was never anything behind the curtain but you.*
- Expertise (2): *Two things you do the way other people breathe.*
- Jack of All Trades (2): *Half of every trade you ever crossed, because everything is material.*
- Font of Inspiration (5): *The show goes on. It always has.*
- Countercharm (7): *Play louder than the thing that is trying to get in.*
- Expertise (9): *Two more. You were always going to collect them.*
- Magical Secrets (10): *Other people's magic, overheard, kept.*
- Superior Inspiration (18): *You are never actually out of material.*
- Words of Creation (20): *At the bottom of every art there is a word. You have two of them now, and one of them is no.*
**College of Dance**

- Dazzling Footwork (3): *Being watched is armour. Being watched is the blow.*
- Inspiring Movement (6): *You move, and because you moved, so do they.*
- Tandem Footwork (6): *You set the tempo of the whole room before anyone has drawn.*
- Leading Evasion (14): *Your partners share your step, including the one out of the fire.*
**College of Glamour**

- Beguiling Magic (3): *Loveliness, and the threat under it. Two faces of one light.*
- Mantle of Inspiration (3): *You dress your friends in a little of the loan.*
- Mantle of Majesty (6): *You speak, and it is done. Try not to enjoy it.*
- Unbreakable Majesty (14): *Too beautiful to strike. They find that out mid-swing.*
**College of Lore**

- Bonus Proficiencies (3): *Three more things went in the bag.*
- Cutting Words (3): *A word placed exactly, subtracted from the world's roll.*
- Magical Discoveries (6): *You watched how the priests, the druids and the wizards do it. Two of their tricks are yours now, and they have not noticed.*
- Peerless Skill (14): *You have read how this is done. Now you do it.*
**College of Valor**

- Combat Inspiration (3): *The war-chant turns the axe, or drives it.*
- Martial Training (3): *The sword is an instrument. You tuned it.*
- Extra Attack (6): *A verse, then a blow, in one measure.*
- Battle Magic (14): *Spell, then steel, before the echo dies.*

## Brand-Voice

**2. The voice the frame should borrow**

- Home lede: *A stranger, a sheet, and a seed you can hand to anyone.*
- Empty state: *The page is waiting for a person.*
- Error: *That one did not come through. Try another seed.*
- Share status: *Link copied. The seed is in it.*

## Cleric

**Core lessons**

- Spellcasting (1): *You asked, and something answered. It has answered every time since, and you have stopped being surprised and never stopped being grateful.*
- Divine Order: Protector (1): *You take after the hand that was held out. Somebody has to stand in front.*
- Divine Order: Thaumaturge (1): *You take after the voice that answered. Somebody has to keep asking.*
- Channel Divinity (2): *The door home was never locked. You learned where the handle is.*
- Sear Undead (5): *What mocks the order of living and dying offends the house you belong to. The house burns it.*
- Blessed Strikes (7): *A hand laid over your hand on the work. It does not do the work. It makes sure it lands.*
- Improved Blessed Strikes (14): *The hand has stopped correcting. It trusts the swing now, and the swing knows it.*
- Divine Intervention (10): *You called home. It answered. It always answers now, and you no longer test it to be sure.*
- Greater Divine Intervention (20): *Ask for anything. That is what it has been waiting to hear you say.*
**Life Domain (the nurturing parent)**

- Disciple of Life (3): *Every mending you were shown, you do a little more of. Care compounds.*
- Preserve Life (3): *The worst hurt first. Nobody past half. That is the mother's arithmetic, and it is not cruelty.*
- Blessed Healer (6): *Mercy feeds the merciful. You did not know that until you had given enough to notice.*
- Supreme Healing (17): *No stint, no ration, no count. Every die comes up full, the way the hand was always full.*
**Light Domain (the teacher who shows the road)**

- Warding Flare (3): *The lamp lifted at the wrong moment for somebody else's aim.*
- Radiance of the Dawn (3): *The road laid bare, and whatever was hiding on it.*
- Improved Warding Flare (6): *The lamp steadies the hand it shelters. The teacher does not only deflect.*
- Corona of Light (17): *You were shown the road. Now you are what the road is seen by.*
**Trickery Domain (the accomplice parent)**

- Blessing of the Trickster (3): *The wink, passed on. Somebody covered for you; you cover for them.*
- Invoke Duplicity (3): *An alibi authored by heaven. You were somewhere else, and heaven will swear to it.*
- Trickster's Transposition (6): *You were never where the trouble looked. Now you can prove it twice in one breath.*
- Improved Duplicity (17): *The joke was a kindness all along. When it ends, somebody is better for it.*
**War Domain (the demanding parent)**

- Guided Strike (3): *Your blow went wide. The correction came, firm as a hand over your hand on the grip.*
- War Priest (3): *One more, because you were told one more, and it was always true.*
- War God's Blessing (6): *The drill instructor takes the field beside you. It has been waiting to.*
- Avatar of Battle (17): *This is what the bar was for. You can take the hit and stand.*
**12. What the Kept seat costs**

- A. Attention: *Somebody is always watching over you. That is the whole of the word.*
- B. Precedence: *You wanted this. You are almost sure you wanted it first.*
- C. Inheritance: *They are keeping you for something. You have started to guess what.*

## Draft-Tables

**A. The Orders' penalty ladder (Q-0027)**

- Minor: *The Order asks for one thing and it is not much, and you will remember it longer than they will.*
- Medium: *You are still one of them. You are not sure they are still yours.*
- Major: *You kept the skills. They kept the name.*
- the Forge: *Everything you made with our mark is unmade or unsigned. Choose which.*
- Home: *The door closes. It is the only thing we have ever closed on anyone.*
- the Hunt: *You are what runs now. We will not be quick about it.*
- Mercy: *We will not touch you. You will find out what that costs.*
- the Veil: *You keep the face. Nobody will believe it again.*
- the Road: *Every threshold we keep is a wall to you. Walk around.*
- the Word: *Your name is taken out of the record. Not struck. Taken.*
- the Storm: *We stop standing between you and the weather.*
- the Shield: *You stand in the last rank, alone, until you learn what the rank was for.*
- the Wall: *You are outside. The Wall does not remember who built it.*
- the Eye: *We will always know where you are. We will never look.*
- the Beast: *The kinship is withdrawn. Whatever you kept will know it before you do.*
**D. One value per written background (Lenses §13)**

- Agent of the Ninth Quill: *You have never once read what you carried. Once.*
- Gambler: *You never leave a table before it is finished.*
- Servant: *You pour for anyone. You have never poisoned a cup, and you know how.*
- Exorcist: *You use what works. You have never used a person.*
- Aberrant Mutant: *You will not let anyone see it change. Anyone.*
- Archaeologist: *You put back the bones. Everything else is yours.*
- Debunker: *You never fake the evidence, even for a true thing.*
- Inquisitor: *You ask before you decide. You always decide.*
- Vagabond: *You do not steal from the house that fed you.*
- Survivalist: *Nobody in your party goes hungry before you do.*
- Dragon Cultist: *You still will not say its name aloud. That is the last thing you kept.*
- Wildkeeper: *You take what you need and you count it.*
- Sellsword: *Paid is paid. You have never switched sides during the job. After is after.*
- Fortune Teller: *You never tell them the good part first.*
- Revolutionary: *You do not name the others. Not under anything.*
- Ice Nomad: *You never leave a fire burning behind you and never let one die in front of you.*
- Guardian: *You do not step out of the doorway. Ever.*
- Herald: *You carry the word exactly. Your opinion travels separately.*
- Naturalist: *You paint what is there. Nobody will ever catch you flattering a tree.*
- Tomb Raider: *You do not open the ones with names on them.*
- Hermeticist: *You write it down before you try it, and you try it.*
- Squire: *You keep the shield. It is not for sale, and it is not yours.*
- Stranger: *You do not ask their names, and you never forget one.*
- Renegade: *You left. You did not take anyone with you, and you never will.*
- Arcane Mutant: *You do not touch people when it is loud in you.*
- Bailiff: *You collected from everyone. You never collected twice.*
- Destined: *You do not tell them to follow. You have never told them to stop.*
- Fated: *You were told how it ends. You have not told anyone else.*
- Survivor: *You say the names every night. You never say them to anyone.*
- Investigator: *You do not stop at the answer that pays.*
- Shadow: *You never let it get between you and a child.*
- Spirit Medium: *You never repeat what the dead say about the living, and they always ask.*

## Dragonborn

**9. Lines**

- Darkvision: *A dragon sees in the dark. So, a little, do you.*
- Draconic Flight: *The ones who stayed home never grew these. You did not ask why. You went.*
**12. Breath Weapon**

- Breath Weapon: *Everything you were taught to hold in, let out once, at something.*

## Druid

**Core lessons**

- Spellcasting (1): *Most of it is listening. The rest the world does for you, because you asked the way it likes to be asked.*
- Druidic (1): *Letters in leaf and stone, from people you will never meet, in a language the cities never learned.*
- Primal Order: Magician (1): *You went further into the asking.*
- Primal Order: Warden (1): *You went further into the standing.*
- Wild Shape (2): *You watched until you could be the thing you watched. The wild lends its bodies the way a friend lends a coat.*
- Wild Companion (2): *The older house sends you company from its own.*
- Wild Resurgence (5): *Speaking with the wild and wearing it turned out to be one currency.*
- Elemental Fury (7): *Whichever hand you use, the same world is behind it.*
- Improved Elemental Fury (15): *The world behind your hand has grown heavier.*
- Beast Spells (18): *You kept your speech inside the borrowed body. The two halves of the craft stopped being halves.*
- Archdruid (20): *Mountains count in winters. You have started to.*
**Circle of the Land**

- Circle Spells (3): *Each country teaches its own way of answering.*
- Land's Aid (3): *The ground feeds its own and turns on the rest, in one gesture.*
- Natural Recovery (6): *Rest in a place you have learned and it gives you back your reach.*
- Nature's Ward (10): *Poison forgets your name. Fear finds nothing in you to hold. Green things part.*
- Nature's Sanctuary (14): *Even the beast, even the briar, turns aside from the one who remembered the terms.*
**Circle of the Moon**

- Circle Forms (3): *Not even flesh is the limit of the vocabulary.*
- Moon Spells (3): *The night's whole company: silver, and teeth.*
- Improved Circle Forms (6): *Moonlight in the claw.*
- Moonlight Step (10): *You move the way light moves.*
- Lunar Form (14): *Phases do not destroy the moon.*
**Circle of the Sea**

- Sea Spells (3): *You came back with its weather in you.*
- Wrath of the Sea (3): *You do not aim the sea. You stand in it, and it happens around you.*
- Aquatic Affinity (6): *The border opened. Breath, and speed, and the cold not minding you.*
- Stormborn (10): *A storm spares its own.*
- Oceanic Gift (14): *One water. It feeds your people and breaks what stands against them, with no hatred in it anywhere.*
**Circle of Stars**

- Star Map (3): *You watched until the watching became a map.*
- Starry Form (3): *The sky lends its figures the way the forest lends its beasts: from a higher pasture, by the same friendship.*
- Cosmic Omen (6): *Read the sky at rest. Then tell luck which way to lean, a breath before it lands.*
- Twinkling Constellations (10): *The figures burn brighter for being read.*
- Full of Stars (14): *Watch anything long enough, with enough love, and you take on its nature.*

## Dwarf

**1. Where the Dwarf lives in the code**

- Metaphysic: *Platonic soul-metal; Sainthood as transcendence into one's own pure substance.*
**9. Lines**

- Darkvision: *The mines taught our eyes to work where the lamps do not reach.*
- Dwarven Resilience: *Every dwarf grows up tasting the mine air and the smelter's fumes. What did not kill our ancestors does not poison us.*
- Dwarven Toughness: *A dwarf is built like a ledger: every year adds a line, and none is ever struck out.*
- Stonecunning: *Lay a hand on the stone and listen. The mountain still keeps our accounts.*

## Elf

**10. Lines**

- Keen Senses: *Your elven eyes are sharp and your attention focused.*
- Fey Ancestry: *The echoes of the Fae still linger in you, letting you see…*
- Trance: *Elves are said to be made of the same essence as dream and nightmare. Perhaps there is more than mere poetry to it.*
- Dark Elf lineage: *The dark taught your people how to survive it, and some of that lesson still answers when you call.*
- High Elf lineage: *Your people traded with more than merchants, and not everything you brought home was cargo.*
- Fae Elf lineage: *The crossing into the Feywild left more on you than long ears.*
- Shadow Elf lineage: *Telling dream from thought took practice, and the practice stayed.*
- Darkvision (proposal, if the Aasimar convention spreads): *The dark is only the Dream with the lights off. You have never been afraid of it.*

## Equipment-and-Masteries

**2. What six loadouts looked like**

- Orc Farmer Rogue, L5: *Studded Leather of the Standing Stone*
**4. Weapon Mastery on the sheet**

- Cleave: *The swing does not stop at the first one.*
- Graze: *Even the miss costs them something.*
- Nick: *The second blade is already moving.*
- Push: *You decide where they stand.*
- Sap: *They will be slower to answer.*
- Slow: *You take the ground out from under their feet, a step at a time.*
- Topple: *Down is a place you can send them.*
- Vex: *You have read them now. The next one lands.*

## Feats

**3. The Dark Gifts**

- Aberrant Anatomy: *Exposure to something from outside has rearranged you.*
- Echoing Soul: *You remember a life that was not this one.*
- Living Shadow: *Your shadow moves on its own, and occasionally on its own behalf.*
- Touch of Death: *Something in you is already partway across.*
- Watchers: *Something is always looking, and it is not on your side.*
- Second Skin: *There is another shape in you, and it does not always wait to be asked.*
- Symbiotic Being: *Something else lives in you, helps you, and is not on your errand.*
**4. The General feats: what the road taught**

- Ability Score Improvement: *The road asked, and the body answered.*
- Alert: *You stopped being surprised. It was cheaper than being brave.*
- Lucky: *Three times in your life the dice were wrong in your favour. You have not asked why.*
- Tough: *You have been hit more than you have been missed, and you are still here.*
- Resilient: *One thing used to get past you. It does not any more.*
- Durable: *You heal like someone who cannot afford to stay hurt.*
- Speedy: *You learned that most trouble is slower than you are.*
- Chef: *Somebody has to feed them. You found out it was you, and you got good at it.*
- Fey-Touched: *Something on the other side of a hedge took an interest. You can step the way it does.*
- Shadow-Touched: *The dark did you a favour once. You have not finished paying.*
- Inspiring Leader: *You say the thing before the fight that lets them walk into it.*
- Keen Mind: *You read faster than trouble arrives, and you remember what you read.*
- Observant: *You watched. That was the whole training.*
- War Caster: *You can hold a spell in one hand and a sword in the other and lose neither.*
- Great Weapon Master: *You stopped swinging carefully. It turned out careful was the problem.*
- Sharpshooter: *Distance is a number, and you learned to ignore it.*
- Sentinel: *Nobody walks past you to get to them.*
- Polearm Master: *You learned the reach, then the butt of it, then the moment they step in.*
- Ritual Caster: *Slow magic, from a book, by candlelight. It works, and nobody stops you.*
- Skill Expert: *One more thing you do the way other people breathe.*
**5. The Epic Boons: level 19, and the cap**

- Boon of Irresistible Offense: *Nothing they are made of turns your blow any more.*
- Boon of Combat Prowess: *Once a round, you decide the miss did not happen.*
- Boon of Dimensional Travel: *Distance stopped applying to you between one thing and the next.*
- Boon of Energy Resistance: *Two kinds of fire chose you, and you choose which two each morning.*
- Boon of Fate: *Luck within sixty feet of you is yours to lean on.*
- Boon of Fortitude: *Forty more of you, and every mending mends more.*
- Boon of Spell Recall: *Sometimes the slot does not spend. You have stopped counting on it and started noticing it.*
- Boon of Recovery: *You have gone down for the last time. It did not take.*
- Boon of Skill: *Everything, a little. One thing, entirely.*
- Boon of Speed: *Thirty more feet, and nothing can hold you.*
- Boon of the Night Spirit: *The dark stopped hiding you and started being you.*
- Boon of Truesight: *You see what is there. All of it. It is not always a gift.*

## Fighter

**7. Drafts: the few lines that need replacing**

- Weapon Mastery (1): *Every weapon you could lift. These are the ones your hands remember.*
- Remarkable Athlete (Champion 7): *The body you built answers first. Everyone else is still deciding.*
- Heroic Warrior (Champion 10): *You start every round with something in reserve. You put it there yourself.*
- Survivor (Champion 18): *Bloodied, and still swinging, and mending as you swing. This is what the post was for.*
- Psionic Power (Psi Warrior 3): *The fight before the fight. You trained that one first.*
- Eldritch Strike (Eldritch Knight 10): *You know how to find a magical weak spot. That's your best trick.*
- Arcane Charge (Eldritch Knight 15): *You are not sure how it works, but it works like a charm.*

## Gnome

**9. Lines**

- Darkvision: *The Fae are said to be part of the Dream. Maybe Gnomes still carry some of it, because you have always felt the night welcomes you.*
- Gnomish Cunning: *Curiosity got your people through worse than a spell, and it still does.*
- Forest Gnome lineage: *You were not supposed to go into the woods, but the Fey felt closer there, and you learnt to listen…*
- Rock Gnome lineage: *You learned early that anything could be taken apart and improved, and somewhere along the way you learned to do it with a word instead of a screwdriver.*

## Goliath

**10. Lines**

- Powerful Build: *You were made smaller than your ancestors, never lighter.*
- Large Form: *For a few minutes you are the size your ancestors never stopped being.*

## Halfling

**7. Lines**

- Brave: *You were brave enough to leave home. You can handle this.*
- Halfling Nimbleness: *This is just like child's play. Try not to get caught.*
- Luck: *Halflings are said to be the luckiest people alive. You hope…*
- Naturally Stealthy: *You read in your stories that halflings can become invisible. You often wish it were true.*

## Human

**9. Lines**

- Versatile (proposal): *Somebody, somewhere along the road, taught you one more thing. You did not always ask what it would cost.*

## Monk

**Core lessons**

- Martial Arts (1): *Nobody can confiscate a fist. You made sure that was enough.*
- Unarmored Defense (1): *Armor is a wall you carry. You would rather not be where the blow lands.*
- Unarmored Movement (2): *The ground is a suggestion. Distance is a habit other people keep.*
- Uncanny Metabolism (2): *You have stood back up in the middle of a fight so many times that your body now does it without you.*
- Deflect Attacks (3): *An arrow is a thing that used to hit you. Now it is a thing you catch, and sometimes return.*
- Slow Fall (4): *A fall is a stretch of air with a floor at the end. You learned to arrive rather than land.*
- Extra Attack (5): *The exchange doubles. You were already counting in twos.*
- Stunning Strike (5): *There is a place on every body where the whole fight stops. Find it once and you can find it again.*
- Empowered Strikes (6): *Ten thousand mornings did what no spell did. Your bare hands stopped being ordinary.*
- Evasion (7): *The fire went where it was aimed. You were not there. You were never going to be there.*
- Acrobatic Movement (9): *Walls are floors that stand up. Water is a floor in a hurry.*
- Heightened Focus (10): *Every technique had a second form. It was waiting for someone who could hold it.*
- Self-Restoration (10): *A mind that stops on fear is caught by it. Yours does not stop. It does not stop for hunger, either.*
- Deflect Energy (13): *Fire and lightning are only faster arrows.*
- Disciplined Survivor (14): *Nothing lands cleanly on you any more. Not poison, not lies, not the ground.*
- Perfect Focus (15): *The well refills when the fight starts. It stopped waiting for you to rest.*
- Superior Defense (18): *For one minute the world can still touch you. It cannot make it count.*
- Body and Mind (20): *The list is finished. There is nothing left on it.*
**Warrior of Mercy**

- Hand of Harm (3): *A knuckle's width from the place that heals is the place that does not.*
- Hand of Healing (3): *The same map, read the other way.*
- Physician's Touch (6): *Both gifts grew. They always grow together. That is the price of learning the whole map.*
- Flurry of Healing and Harm (11): *Mercy at fighting speed: a hand for each of them, and no time to explain which was which.*
- Hand of Ultimate Mercy (17): *There is one door the map does not show. You found it. It opens once a day, and only from this side.*
**Warrior of the Open Hand**

- Open Hand Technique (3): *Stand exactly here. Move exactly now. Where your palm lands, the fight goes.*
- Wholeness of Body (6): *The instrument keeps itself in repair. Nobody else was ever going to.*
- Fleet Step (11): *One technique opens into the next. You stopped noticing the seams years ago.*
- Quivering Palm (17): *The touch that waits. You have already used it. What you have not decided is whether it ends.*
**Warrior of Shadow**

- Shadow Step (6): *Out of one shadow and into another. Between the two, there is no you anywhere at all.*
- Improved Shadow Step (11): *The dark stopped being a requirement. It was only ever a courtesy you paid it.*
- Cloak of Shadows (17): *The night is no longer hiding you. It has started to take your side.*
**Warrior of the Elements**

- Elemental Attunement (3): *The punch arrives as flame, ten feet on. The world moves with your wrist.*
- Manipulate Elements (3): *A small conversation with the weather. It answers in kind.*
- Elemental Burst (6): *The form's big release. You have been holding it since the first stance.*
- Stride of the Elements (11): *The element carries you now: up through the air, down through the sea.*
- Elemental Epitome (17): *Everything you touch is touched by the element. Everything that touches you learns why.*

## NPCs-and-Villains

**4. Personality: 4,209 lines, gated by alignment**

- Bandit: *Not from the ones who fed us. Everyone else is weather.*
- Pirate: *The ship is the country. The rest is coastline.*
- Spy: *I have never lied to the one who pays. Ask me who that is.*
- Cultist: *I believe it. That is the part they cannot forgive.*
- Priest: *I stopped believing years ago. The people did not, and they are my job.*
- Witch: *I keep the bargains. Read them before you sign.*
- Knight: *I gave my word once. Everything since is the shape of keeping it.*
- Noble: *The house before the name. The name before me.*
- Merchant: *Everything has a price. I have never once sold the thing that does not.*
- Mentor: *I will teach you everything, and one thing I will keep, and you will hate me for the right one.*

## Orc

**1. Where the Orc lives in the code**

- Metaphysic: *The wind path: a route each soul walks rather than a substance it is made of.*
**10. Lines**

- Adrenaline Rush: *The winds of your storm are hard to catch, rider. Run, be free, and run.*
- Darkvision: *The ride does not end at nightfall, and neither does your watch.*
- Relentless Endurance: *Do not fall, rider! Be strong! Carry on!*

## Orders

**8. Relationships: the lost layer, redrafted**

- MEANS: *{creed_a} That is the end. {creed_b} That is how you get there, and the house does not let you forget which is which.*
- RESTRAINT: *{creed_a} {creed_b} The second is there to stop the first from becoming everything. It has not always managed.*
- MASK: *To the town, {creed_a} Inside the walls, {creed_b} The house wears the first so that it can keep the second.*
- DEBT: *{creed_a} Because of that, {creed_b} The one was owed before the other could be taught, and the house is still paying.*
- WOUND: *{creed_a} The house learned that the hard way, and it learned the second thing from the scar: {creed_b}*
- THRESHOLD: *{creed_a} That is the door. {creed_b} That is what waits on the other side of it, and nobody is told which side they are standing on.*
- REVELATION: *{creed_a} You were taught that first, and believed it. Then, one night: {creed_b} You have not decided whether the first was a lie or a ladder.*

## Paladin

**Feature lines: core**

- Lay on Hands (1): *A pool of mending from nothing but the word. You said you would. So you can.*
- Spellcasting (1): *The magic is the strength of the one who swore. It is exactly as strong as you are.*
- Weapon Mastery (1): *The vow needs a hand it can land through.*
- Fighting Style (2): *You chose a way to stand, and you have stood that way since.*
- Paladin's Smite (2): *The word goes into the blow. Whatever you hit finds out what you promised.*
- Channel Divinity (3): *You can feel what the oath is against. It is never far.*
- Extra Attack (5): *Twice, because once was a wish and the vow is not a wish.*
- Faithful Steed (5): *Something answered the oath and agreed to carry it. You did not ask what kind.*
- Aura of Protection (6): *Stand near you and the vow covers them too. That is what it was for.*
- Abjure Foes (9): *Whatever the oath is against knows it, and steps back.*
- Aura of Courage (10): *You have already decided. Deciding is contagious.*
- Radiant Strikes (11): *Every blow carries the word now, not only the ones you spend on.*
- Restoring Touch (14): *Your hand gives people back to themselves. Somebody once did that for you, and you swore.*
- Aura Expansion (18): *The circle widens. The vow has held more people than it was made for, and it grew.*
**Oath of Devotion**

- Sacred Weapon (3): *Your blade gives light. You could not bear to fight in the dark.*
- Aura of Devotion (7): *Nobody near you can be talked into being someone else.*
- Smite of Protection (15): *The word lands, and it shields the one it struck through.*
- Holy Nimbus (20): *There is nowhere near you left to hide, including from yourself.*
**Oath of the Ancients**

- Nature's Wrath (3): *What was here first still has hands.*
- Aura of Warding (7): *New magic breaks against the old. You are standing on the old.*
- Undying Sentinel (15): *The old things do not age. Neither, quite, do you. It was not asked for.*
- Elder Champion (20): *The green comes back. For a minute, it comes back through you.*
**Oath of Glory**

- Inspiring Smite (3): *The deed feeds the ones who saw it.*
- Peerless Athlete (3): *Nine or lower is a number for people who are not being watched.*
- Aura of Alacrity (7): *The crowd runs with you. It always has.*
- Glorious Defense (15): *Another's failure, turned into your story, and their survival.*
- Living Legend (20): *Whether or not it happened that way, it is told that way, and the telling is armour.*
**Oath of Vengeance**

- Vow of Enmity (3): *One name. When it falls, the vow does not; it moves.*
- Relentless Avenger (7): *They cannot run. You made sure of that a long time ago.*
- Soul of Vengeance (15): *They cannot swing without an answer.*
- Avenging Angel (20): *The wings come. You find out what the wrong made of you.*
**11. Oath Spells**

- Oath Spells: *The oath comes with a vocabulary. These are the words it lets you say.*

## Ranger

**Feature lines: core**

- Favored Enemy (1): *You named it. The naming does not let go.*
- Weapon Mastery (1): *Two ways of ending a pursuit, and you know which one the ground wants.*
- Spellcasting (1): *The wild lends you a little of what the druids get. You spend it on arriving.*
- Deft Explorer (2): *A second tongue and one thing you do better than anyone, learned on the road because the road demanded it.*
- Fighting Style (2): *You chose how to stand when it turns and comes at you.*
- Extra Attack (5): *Twice, because the first one told you where it was.*
- Roving (6): *Cliff, river, ten more feet. Nothing between you and it counts as terrain any more.*
- Expertise (9): *Two more things you do better than anyone. The road kept demanding.*
- Tireless (10): *You walk on something that is not sleep. It holds.*
- Relentless Hunter (13): *Pain does not shake the mark loose. Nothing does.*
- Nature's Veil (14): *Step into the hedge and stop existing.*
- Precise Hunter (17): *You see it better than anything else in the world.*
- Feral Senses (18): *You can find what you cannot see. It has not been out of reach since you named it.*
- Foe Slayer (20): *The mark goes deeper. It was always going to.*
**Hunter**

- Hunter's Lore (3): *You know what it is made of before it knows you are there.*
- Hunter's Prey (3): *Break the horde, or answer the giant. You pick at dawn.*
- Defensive Tactics (7): *Step out of the crowd untouched, or refuse to be knocked down by it.*
- Superior Hunter's Prey (11): *The mark spreads to whatever is standing next to it.*
- Superior Hunter's Defense (15): *Turn the blow so it lands on the one who swung.*
**Beast Master**

- Primal Companion (3): *Something walked out of the wild and chose you.*
- Exceptional Training (7): *It learned to run, dodge and help on your word alone. It learned that for you.*
- Bestial Fury (11): *Twice, and harder on the one you marked. It hunts the mark with you.*
- Share Spells (15): *What you cast on yourself, you cast on both of you. There was never a difference.*
**Gloom Stalker**

- Dread Ambusher (3): *The first breath of a fight is yours. You were moving before it started.*
- Gloom Spells (3): *A different face, a hole in the air, a fear you can hand out.*
- Umbral Sight (3): *Whatever sees in the dark cannot see you at all.*
- Iron Mind (7): *What hunts at night gets into heads. Not yours.*
- Stalker's Flurry (11): *A miss is only the first half of the strike.*
- Shadowy Dodge (15): *You were not quite where the blow arrived. You rarely are.*
**Fey Wanderer**

- Dreadful Strikes (3): *Something of the court is in the blow, and it does not let a wound close.*
- Fey Spells (3): *A charm, a step, a summons, a door, a lie. Souvenirs.*
- Otherworldly Glamour (3): *A little too charming for the woods.*
- Beguiling Twist (7): *A charm turned back on whoever cast it, and aimed.*
- Fey Reinforcements (11): *Friends who arrive when you whistle and are not entirely made of anything.*
- Misty Wanderer (15): *A step that crosses distances the woods do not have, with room for one more.*

## Rogue

**Feature lines: core**

- Expertise (1): *Two things you do the way a lock does: without thinking, and every time.*
- Sneak Attack (1): *The gap. You were already in it.*
- Thieves' Cant (1): *A conversation about the weather that is not about the weather.*
- Weapon Mastery (1): *A blade that does one extra thing, because one extra thing is usually enough.*
- Cunning Action (2): *You were moving before the turn started. That is not a trick. That is the trade.*
- Steady Aim (3): *Stand still and the moment comes to you.*
- Cunning Strike (5): *Spend a little of the blow to shape it. A knee, a whisper, a stumble.*
- Uncanny Dodge (5): *The blow that should have landed found half of you gone.*
- Expertise (6): *Two more. The trade keeps asking.*
- Evasion (7): *The fire went where it was aimed. You had already left.*
- Reliable Talent (7): *Nine or lower is a ten. The work is never a gamble; that is what makes it work.*
- Improved Cunning Strike (11): *Two shapes on one blow.*
- Devious Strikes (14): *A daze, a sleep, a blinding. The blow has learned manners.*
- Slippery Mind (15): *Nobody talks you into anything. You have been on the other side of that conversation too often.*
- Elusive (18): *Nobody gets the drop on the one who lives by getting the drop.*
- Stroke of Luck (20): *The roll that should have missed, and did not. You call it luck in front of strangers.*
**Thief**

- Fast Hands (3): *Your hands finished before your turn did.*
- Second-Story Work (3): *A wall is a staircase. A window is a door that was not expecting you.*
- Supreme Sneak (9): *Strike from hiding and stay hidden, if you end where the wall is.*
- Use Magic Device (13): *You have never understood a scroll in your life. They work anyway.*
- Thief's Reflexes (17): *Two turns in the first breath. By the second one they know; by then you were not there.*
**Assassin**

- Assassinate (3): *The first breath of the fight is the only breath that matters.*
- Assassin's Tools (3): *A face for every room and a poison for every wound.*
- Infiltration Expertise (9): *An hour of watching, and you can write in their hand.*
- Envenom Weapons (13): *The poison does not care what it was told to resist.*
- Death Strike (17): *You do not fight. You finish.*
**Arcane Trickster**

- Spellcasting (3): *Magic that was lying around, and somebody was not watching it.*
- Mage Hand Legerdemain (3): *A second hand nobody can see, and it has your fingers.*
- Magical Ambush (9): *A spell from where nobody is looking lands harder.*
- Versatile Trickster (13): *The invisible hand trips one, and the one beside them.*
- Spell Thief (17): *You caught it in flight. It is yours now.*
**Soulknife**

- Psionic Power (3): *A well of something sharp, and a crew that speaks without speaking.*
- Psychic Blades (3): *The blade is a thought. It goes through armour the way an idea goes through a door.*
- Soul Blades (9): *A miss turns, mid-air. A step through the place where a room is thin.*
- Psychic Veil (13): *Static, worn like a coat. Nobody can find the thought that is you.*
- Rend Mind (17): *The blade goes in where the thinking is, and the thinking stops.*

## Sheet-Alignment-Languages

**2. Alignment: two axes, and the house names**

- Good: *You will break any rule for a person. You will not break a person for a rule.*
- Neutral: *Nobody tells you. That is the whole position.*
- Evil: *You want what you want, and the wanting is the only law you recognise.*

## Sorcerer

**5. Relationships: species × Sorcerer**

- Dwarf: *"A dwarf who spends ninety years on one gem may Ascend through the obsession itself… the family will describe it as a curse."*
**Feature lines: core**

- Spellcasting (1): *It happened once, and it keeps happening. The magic is exactly as strong as you are.*
- Innate Sorcery (1): *For one minute you stop holding it in.*
- Font of Magic (2): *Not a list. A tide. You can pour it into a shape or pour the shape back into the tide.*
- Metamagic (2): *A spell is a shape somebody else decided on. Yours was never anybody else's, so it bends.*
- Sorcerous Restoration (5): *A body recovers. So does this.*
- Sorcery Incarnate (7): *Turned up, it bends twice.*
- Arcane Apotheosis (20): *You and the magic stopped being two things. It shows.*
**Draconic**

- Draconic Resilience (3): *Scales nobody can see. A blade finds you harder than it should.*
- Draconic Spells (3): *The spells arrive in the same colour as the taste in your mouth.*
- Elemental Affinity (6): *Your own element does more in your hands, and less to you.*
- Dragon Wings (14): *They come. They stay until you say otherwise.*
- Dragon Companion (18): *Something answers that is shaped like what you are turning into.*
**Wild Magic**

- Wild Magic Surge (3): *Some days the sky does something nobody asked for.*
- Tides of Chaos (3): *Lean on the weather. It will lean back, soon, on its own terms.*
- Bend Luck (6): *A finger's width on somebody else's fortune.*
- Controlled Chaos (14): *Two storms offered. You take the one you like.*
- Tamed Surge (18): *You choose which storm arrives. Tamed is the wrong word, and it is the word.*
**Clockwork**

- Clockwork Spells (3): *The wheel under the wheel lends you what it uses to keep turning.*
- Restore Balance (3): *Where luck is being helped or hindered, you make it plain again.*
- Bastion of Law (6): *A wall of pure order, exactly as thick as you paid for.*
- Trance of Order (14): *For a minute, nine or lower is not a number that happens.*
- Clockwork Cavalcade (18): *The mechanism sweeps through, and everything is put back where it goes.*
**Aberrant**

- Psionic Spells (3): *None of it makes a sound any more.*
- Telepathic Speech (3): *Into a mind, without opening your mouth. It learned that first.*
- Psychic Defenses (6): *What looked in cannot be looked into. Neither, now, can you.*
- Revelation in Flesh (6): *Your body has ideas. You did not teach it.*
- Warping Implosion (14): *You leave, and the place you left comes with you.*

## Spells-and-Invocations

**4. The Warlock's clauses**

- Pact of the Blade: *The weapon is the receipt.*
- Pact of the Chain: *The familiar reports to both of you.*
- Pact of the Tome: *The book was written before you opened it.*
- Agonizing Blast: *The clause about pain was your addition.*
- Eldritch Mind: *Nothing gets between you and the thought. Not even the knife.*
- Devil's Sight: *You were issued eyes for where they keep things.*
- Armor of Shadows: *The dark holds you the way a coat does: because it was told to.*
- Fiendish Vigor: *A little more life each time. Lent.*
- Mask of Many Faces: *Whose face you wear is in the terms. Which one is not.*
- Misty Visions: *You show them what is not there. They do the rest.*
- Repelling Blast: *Distance is the one gift you can give them.*
- Ascendant Step: *Up is a direction you were given. Not a promise.*
- Gaze of Two Minds: *You borrow a pair of eyes and give nothing back. Yet.*
- One with Shadows: *Stand still in the dark and you are part of it. Move, and you are yours again.*
- Thirsting Blade: *The blade wants twice what you asked. So did you.*
- Whispers of the Grave: *The dead answer. They were told to.*
- Lifedrinker: *What it takes, you keep. That was the clause.*
- Witch Sight: *You see what things are. It has not made you happier.*
- Lessons of the First Ones: *One lesson from before the terms. They let you keep it.*
- Gift of the Protectors: *Names in the book. Whoever is written there falls a little slower.*
**5. The spellcasting section itself**

- Wizard: *What you know, you took. The book is where you keep the receipts.*
- Cleric: *You do not cast these. You ask, and they are answered, and you have stopped being surprised.*
- Druid: *The land lends. You have learned how to ask, and how to give it back.*
- Sorcerer: *It was in you before you had a word for it. The words came later, and they are still catching up.*
- Warlock: *Pact Magic. The name is exact. Read the terms.*
- Bard: *A song is a spell that admits what it is. These are the ones that do not.*
- Paladin: *The oath speaks. This is what it says when it has to.*
- Ranger: *What the edge of the map taught, in the only language it has.*
- Artificer: *Magic is a component. You know where it goes.*
- Eldritch Knight: *A careful study, between drills.*
- Arcane Trickster: *Small magic, well placed. The big kind gets you noticed.*

## Stories-and-Titles

**8. What the story should be**

- Dwarf: *Whatever {name} is made of, the road is finding out.*
- Elf: *{name} has been away long enough to start looking like somewhere else.*
- Aasimar: *The light in {name} does not know what {name} intends. It has not asked.*
- Tiefling: *{name} has never done anything. Everyone who meets {hero} already knows what {name} did.*
- Dragonborn: *{name} left the house. The house is still there, and so is the door.*
- Orc: *The wind that took {name} from {hometown} has not yet said where it is going.*
- Goliath: *{name} made a promise once. The road is the shape of keeping it.*
- Halfling: *{name} knows the way back to {hometown}. Knowing is enough.*
- Gnome: *{name} has an idea. This is usually where the trouble starts.*
- Human: *{name} owes half the people in {kingdom} a favour and is owed by the other half. That is the map.*

## Tiefling

**10. Lines**

- Darkvision: *Nobody ever lit a lamp for you. You learned to do without.*
- Otherworldly Presence: *A little theatre. It is the one thing they expect from you, so you learned to use it.*
- Fiendish Legacy: Infernal: *The fire was always there. What it is for was never decided by them.*
- Fiendish Legacy: Abyssal: *The colours move while you sleep. So, some mornings, does what you can do.*
- Fiendish Legacy: Chthonic: *No fire, no pain. Only the door, and you know which side of it you stand on.*

## Warlock

**Core lessons**

- Pact Magic (1): *The slots are few because they are lent, and they come back because the lender wants them used.*
- Eldritch Invocations (1): *Pieces of something you were not supposed to know, and now cannot not know.*
- Pact of the Blade (1, invocation): *It gave you a weapon so that it would not have to come itself.*
- Pact of the Chain (1, invocation): *It sent something small to keep an eye on the investment. The small thing likes you. That was not in the terms.*
- Pact of the Tome (1, invocation): *A book in a hand you almost recognise. Some pages were written after you opened it.*
- Contact Patron (9): *You used to go through intermediaries. Now it picks up. You are not sure that is an improvement.*
- Mystic Arcanum (11): *A secret too large for the slots. Once a day, it lets you say it.*
- Eldritch Master (20): *When the fight starts, the secret comes back on its own. It has stopped waiting for you to ask.*
**The Fiend (interior)**

- Dark One's Blessing (3): *Something dies near you and you feel better. You have stopped noticing that you notice.*
- Dark One's Own Luck (6): *A number out of nowhere, when you need one. Luck is not free. Somebody, somewhere, just had a bad day.*
- Fiendish Resilience (10): *You chose what cannot hurt you today. You will choose again tomorrow. You have not yet chosen the thing that actually can.*
- Hurl Through Hell (14): *There is a door in you. You have never gone through it. Somebody else does, every time.*
**The Celestial (the briefing)**

- Healing Light (3): *Standard issue. A metered pool, and you spend it on the others. Do not ask what happens if you run it dry on yourself.*
- Radiant Soul (6): *Same fire as the other lot's, different branding. Try not to think about that.*
- Celestial Resilience (10): *You get topped up after a rest, and five of your people get half. That is the whole benefits package. Read it twice.*
- Searing Vengeance (14): *Somebody near you is about to die and the office objects. You are the objection. Everyone standing too close finds out.*
**The Archfey (couplets)**

- Steps of the Fey (3): *Step, and be a step away.<br>Which you take is ours to say.*
- Misty Escape (6): *Struck, and gone, and not yet found.<br>Fear stays where you left the ground.*
- Beguiling Defenses (10): *Charm us? We invented charm.<br>Hurt you, and we hand back harm.*
- Bewitching Magic (14): *Cast the lie, and leave the place.<br>Let them argue with your face.*

## Wizard

**Feature lines: core**

- Spellcasting (1): *Study is the discipline of noticing twice.*
- Ritual Adept (1): *Some spells are read, not remembered. Slower, and the book does not mind.*
- Arcane Recovery (1): *An hour with the book gives some of the day back. Memory is a well, not a jar.*
- Scholar (2): *One field where the reading went deeper than magic.*
- Memorize Spell (5): *An hour, a page, and a different answer ready by evening.*
- Spell Mastery (18): *Two spells you no longer look up. The argument against forgetting, won twice.*
- Signature Spells (20): *Two more, by heart. The book stays closed and the spell arrives anyway.*
**Abjurer**

- Abjuration Savant (3): *The school of no. The book fills with it, free.*
- Arcane Ward (3): *A ward that takes what was coming for you, and is glad to.*
- Projected Ward (6): *It stands around a friend now. It learned that from you.*
- Spell Breaker (10): *Mend and unmake in one motion. They were always one motion.*
- Spell Resistance (14): *Other people's magic breaks against you like weather on a roof.*
**Diviner**

- Divination Savant (3): *The school of seeing. Two more windows, free.*
- Portent (3): *Numbers behind your eyes at dawn, held for later.*
- Expert Divination (6): *Seeing gives back a little of what it cost.*
- The Third Eye (10): *The second eye, opened.*
- Greater Portent (14): *Three numbers now. The sky is generous with those who read it.*
**Evoker**

- Evocation Savant (3): *The school of aim.*
- Potent Cantrip (3): *It bites even when it misses.*
- Sculpt Spells (6): *Your friends stand inside the blast and are not touched. That is the whole art.*
- Empowered Evocation (10): *A little more of your mind in every burst.*
- Overchannel (14): *All of it, once, free. All of it again, and pay.*
**Illusionist**

- Illusion Savant (3): *The school of what is not there.*
- Improved Illusions (3): *No voice needed, and it can be made to stay.*
- Phantasmal Creatures (6): *Give the lie a body and it will act in your name.*
- Illusory Self (10): *The blow lands on the one of you that was never there.*
- Illusory Reality (14): *What was not there, made to be, for a minute.*
**Bladesinger**

- Training in War and Song (3): *A blade that is also the focus. The pen and the sword were one tool.*
- Bladesong (3): *The sword goes where the thought goes.*
- Extra Attack (6): *Two strokes, then a word.*
- Song of Defense (10): *A spell spent to not be hit. Memory, traded for a moment.*
- Song of Victory (14): *The mind lands behind the blade now.*

