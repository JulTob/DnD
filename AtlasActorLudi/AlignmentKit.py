...
def Find_Alignment(
	...
	morality = (
		"Good" if target in Good
		else "Evil" if target in Evil
		else "Neutral"
		)
	order = (
		"Lawful" if target in Lawful
		else "Chaotic" if target in Chaotic
		else "Neutral"
		)
	if order == "Neutral":
...
def _alignment_tags(
		requested,
		) -> tuple[type[Alignment], ...]:
	words = _alignment_words( requested )
	morality = tuple(
...
def New_Alignment(
	...
	selected_tags = (
		_random_alignment_tags( character )
		if is_random
		else _alignment_tags( alignment )
		)

	for requested_tag in selected_tags:
		existing_tag = _ALIGNMENT_CONFLICTS[requested_tag]
...
def _test_membership_is_source():
	...
	assert Find_Alignment( character ) == "Chaotic Good"


def _test_axis_conflicts():
	from TagKit import TagPreconditionError

	character = Character( seed=20 )
...
def _test_alignment_inputs():
	lawful = Character( seed=21 )
	chaotic = Character( seed=22 )
...
def _self_test():
	...
	print('OK — AlignmentKit self-test')
