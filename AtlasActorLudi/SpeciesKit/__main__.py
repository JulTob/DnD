	...
	for feature in features:
		...
		for forbidden in (
			...
			assert forbidden not in description, (
				f"{feature.name!r} announces itself as a future feature: {forbidden!r}"
				)
...
def _test_playable_species() -> None:
	for species in PLAYABLE_SPECIES:
		...
		if species in HERITAGES_BY_SPECIES:
			assert Current_Heritage(
				character,
				) in HERITAGES_BY_SPECIES[
				species
				]
			assert Find_Heritage(
				character,
				)
			assert Find_Subspecies(
				character,
				) == Find_Heritage(
				character,
				)
			assert f"{character:Subspecies}" == Find_Heritage(
				character,
				)
		else:
...
def _test_random_species_is_level_stable() -> None:
	...
	first_species = Apply_Species(
		first,
		)
	progressed_species = Apply_Species(
		progressed,
		)
...
def _test_human_stateful_grants() -> None:
	...
	assert skillful_choice.name in skillful_entry.description
...
def _test_tiefling_rule_projection() -> None:
	...
	for heritage, progression in expected_spells.items():
		for level, unlocked_count in unlock_counts.items():
			...
			unlocked_legacy = progression[ : unlocked_count ]
			expected_all = (
				"Thaumaturgy",
				*unlocked_legacy,
				)
			expected_free_casts = {
				spell_name: 1
				for spell_name in unlocked_legacy[ 1 : ]
				}
			legacy_feature = next(
				feature
				for feature in character.features
				if feature.name == f"Fiendish Legacy: {heritage.__name__}"
				)
			species_features = tuple(
				feature
				for feature in character.features
			...
			assert character.fiendish_legacy_free_casts == expected_free_casts
			assert character.species_spell_free_casts == expected_free_casts
...
def _test_elf_rule_projection() -> None:
	...
	for heritage, spells in expected.items():
		...
		assert entries[ "Darkvision" ].description == (
			"*Your elven eyes are sharp and your attention focused.*\n\n"
			+ Darkvision_Rules(
				heritage.DARKVISION_RANGE,
				)
			)
		assert entries[ "Fey Ancestry" ].description == (
			"*The echoes of the Fae still linger in you, letting you see through trickery and deception.*\n\n"
			"You have Advantage on saving throws you make to avoid or end the Charmed condition."
			)
		assert entries[ "Keen Senses" ].description == (
			"*Your focus and training can help you perceive your surroundings and understand your environment.*\n\n"
			f"You have proficiency in {character.keen_senses_skill}."
			)
		assert entries[ "Trance" ].description == (
			"*Elves are said to be made of the same essence as dream and nightmare. Perhaps there is more than mere poetry to it.*\n\n"
			"You don't need to sleep, and magic can't put you to sleep. You can finish a Long Rest in 4 hours by meditating."
			)
		assert all(
			any(
				feature.name == name
...
def _test_all_elves_share_common_trait_voice() -> None:
	...
	for heritage in ELF_HERITAGES:
		...
		assert f"within {heritage.DARKVISION_RANGE} feet" in darkvision.description
...
def _test_wood_elf_lineage_progression() -> None:
	...
	for level, expected in expected_by_level.items():
		...
		assert "**Spellcasting Ability.**\n\nIntelligence." in lineage.description
		assert lineage.description.startswith(
			"*The woods have always cared for your people. Now their whispers guide you, their paths open before you, and their shadows protect you.*"
			)
		assert all(
		...
		assert (
			"these spells prepared" in lineage.description
			) is (
			level >= 3
			)
...
def _test_elf_identity_is_level_stable() -> None:
	...
	assert first.species_spellcasting_ability == (
