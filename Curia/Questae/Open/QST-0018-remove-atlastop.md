...
## ✅ Resolution
- **Decided by:** Julio (Decree 0002; closure confirmed 2026-07-15 — "Shouldn't AtlasTOP be eliminated by now?")
- **What changed:** `AtlasTOP/` is gone from the working tree, its two Grimoire call sites and `tests/test_top_integration.py` with it. Verified 2026-07-15: the directory does not exist, zero references anywhere in Python code. Notably, `git log -- 'AtlasTOP/*'` is empty — the package was **never tracked by git**; it lived and died as an untracked working-tree directory, which is why no removal commit exists to link.
- **Practice/preference to remember:** the second half of the Desired outcome ("the Grimoires apply their tags directly with TagKit; `kind` becomes the Player/Non role tag") was **not** done here and is not owed here — it belongs to QST-0016.3, and the first sanctioned in-repo TOP usage landed via QST-0031.1 (SpellsKit) instead. When an unsanctioned layer is cut, cut it fully and let the sanctioned replacement arrive on its own quest — don't smuggle the replacement in through the deletion.
