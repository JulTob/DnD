...
- 2026-08-31 11:32 | Cursor (Claude Opus) | COMMIT | cherry-pick onto `main` | Julio chose: keep their gate-verified guild/species commits; land only the irreplaceable Cleric voice + Questae/REW. Done in a detached worktree so the 914-file index was not touched. `ClericKit` keeps their `SPECIALIZATIONS = Cleric.SPECIALIZATIONS` and only adds `bind_cleric_voice`.
